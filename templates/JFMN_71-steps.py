...
@given('Database is {database_name} added to AOAG with primary replica on {primary_ip} and secondary replica on {secondary_ip}')
def step_impl(context, database_name, primary_ip, secondary_ip):
    context.database_name = database_name
    context.primary_ip = primary_ip
    context.secondary_ip = secondary_ip


@given('Table {table_name} is created on {primary_ip}')
def step_impl(context, table_name, primary_ip):
    context.table_name = table_name
    context.primary_ip = primary_ip
    create_table_result = mssql.create_table(context.primary_ip, context.database_name, context.table_name)
...
def step_impl(context):
    ...
    if context.sla_id.lower() == "invalid":
        db_id = "invalid_id"
    context.tlog_backup_status = rubrik.mssql_tlog_backup(db_id)

@then('the method returns trigger t-log backup status {trigger_tlog_backup_status}')
def step_impl(context, trigger_tlog_backup_status):
    print("Trigger Backup status:", context.tlog_backup_status)
    assert context.tlog_backup_status["status"] == trigger_tlog_backup_status, f"Expected {trigger_tlog_backup_status}, but got {context.tlog_backup_status['status']}"


@when('the method to get the t-log backup status is called')
def step_impl(context):
    output = rubrik.get_mssql_db_id_details(context.database_name)
    db_id = output["data"]["mssqlDatabases"]["edges"][0]["node"]["id"]
    ...
    if status.lower() == "success" or status.lower() == 'partial_success':
        context.final_backup_status = "T-log backup successful"

    elif status.lower() == "failure" or status.lower() == "failed":
        context.final_backup_status = "Backup failed"
    else:
        context.final_backup_status = status

@then('the method returns t-log backup status {tlog_backup_status}')
def step_impl(context, tlog_backup_status):
    assert context.final_backup_status == tlog_backup_status, f"Expected {tlog_backup_status}, but got {context.final_backup_status}"
...
def step_impl(context, table_name):
    ...
    insert_data = mssql.insert_test_data(context.server_ip, context.database_name, context.table_name, backup_type)
    print(f"Second row insertion into {table_name} result: {insert_data}")
    if insert_data.lower() == "test data inserted successfully":
        table_data = mssql.print_all_inserted(context.server_ip, context.database_name, context.table_name)
        print(f"Table data :-\n{table_data}")
        inserted_table_data = table_data
...
def step_impl(context):
    ...
    if context.sla_id.lower() == "invalid":
        db_id = "invalid_id"
    context.full_backup_status = rubrik.mssql_full_backup(db_id, context.database_name)
...
def step_impl(context, database_name, table_name):
    context.db_exists_status = mssql.get_database(context.server_ip, context.database_name)
    ...
    # context.table_exists_status = mssql.get_table(context.server_ip, context.database_name, context.table_name)
